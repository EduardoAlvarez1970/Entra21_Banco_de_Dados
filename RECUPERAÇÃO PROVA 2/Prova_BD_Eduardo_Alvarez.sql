...
# aclaração: a base de dados so tem items com data de venda entre 2016 e 2019 (???) entao nao aparecem dados de 2023, se consultar numa outra data se tem resutados.
# ex 2018:
...
INNER JOIN tabela_de_produtos
ON itens_notas_fiscais.CODIGO_DO_PRODUTO = tabela_de_produtos.CODIGO_DO_PRODUTO
WHERE notas_fiscais.DATA_VENDA >= '2018-01-01' AND notas_fiscais.DATA_VENDA <= '2018-12-31'
GROUP BY itens_notas_fiscais.CODIGO_DO_PRODUTO, tabela_de_produtos.NOME_DO_PRODUTO
HAVING SUM(itens_notas_fiscais.QUANTIDADE) > 1000;
